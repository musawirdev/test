...
async function testTelegramConnection() {
    ...
    try {
        const testMessage = `🧪 **Connection Test Successful!**\n\n` +
            `✅ Raja CC Checker v2.0 is connected\n` +
...
async function redeemApiKey() {
    ...
    try {
        const response = await fetch('/api/c?action=redeem', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function checkCredits() {
    ...
    try {
        ...
        const response = await fetch(`/api/c?${params}`);
        const result = await response.json();
...
function saveApprovedCards() {
    ...
    let content = `═══════════════════════════════════════════════\n`;
    content += `🚀 RAJA CC CHECKER v2.0 - APPROVED CARDS\n`;
    content += `═══════════════════════════════════════════════\n`;
    ...
    content += `❤️ Generated by Raja CC Checker v2.0\n`;
    content += `═══════════════════════════════════════════════\n`;
    ...
    a.href = url;
    a.download = `raja_approved_${timestamp}.txt`;
